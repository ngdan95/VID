...
function calculateVID() {
    ...
    BMI  = $("#qty_weight").val()/Math.pow($("#qty_height").val(),2)
    ...
    ClVanco2 = 9.656 - (0.078 * ($("#qty_age").val())) - (2.009 * (($('#qty_creatinine').val())) / 88.42) + 1.09 + (0.04 * (Math.pow($("#qty_weight").val(), 0.75)));
    //ClVanco2 ClVanco béo phì nam
    ClVanco3 = 9.656 - (0.078 * ($("#qty_age").val())) - (2.009 * (($('#qty_creatinine').val())) / 88.42) + (0.04 * (Math.pow($("#qty_weight").val(), 0.75)));
    //ClVanco3 ClVanco béo phì nữ (không có + 1.09)

    //check element
    console.log(0.078 * ($("#qty_age").val()))
    //check tuoi
    console.log(2.009 * (($('#qty_creatinine').val())) / 88.42)
    //check creatinine
    console.log(0.04 * (Math.pow($("#qty_weight").val(), 0.75)))
    //check function cuoi
    var qty_clvanco;
...
function Calculatenew() {
    const qty_moddose = $('#qty_moddose').val();
    const qty_clvanco = parseFloat($('#qty_clvanco').text())
    const qty_moddi = $('#qty_moddi').val();

    qty_auc5 = Math.round(qty_moddose * 24 / qty_moddi) / qty_clvanco;
    qty_tgt5 = Math.round(qty_moddose / 10 / 60);
    document.getElementById('qty_auc5').innerHTML = qty_auc5;
}
